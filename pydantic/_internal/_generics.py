...
from ._utils import lenient_issubclass, all_identical
from ._typing_extra import get_type_hints, get_args, typing_base
...
def iter_contained_typevars(v: Any) -> Iterator[TypeVarType]:
    ...
    if isinstance(v, TypeVar):
        ...
        hasattr(v, '__parameters__') and not typing_extensions.get_origin(v) and lenient_issubclass(v, BaseModel)
    ):
...
def replace_types(type_: Any, type_map: Mapping[Any, Any]) -> Any:
    ...
    from pydantic import BaseModel
    if not origin_type and lenient_issubclass(type_, BaseModel) and not type_.__concrete__:
