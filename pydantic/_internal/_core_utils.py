...
def consolidate_refs(schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
    ...
    def _extract_definitions(s: core_schema.CoreSchema) -> core_schema.CoreSchema:
        if 'ref' in s:
            if s['ref'] not in definitions:
                definitions[s['ref']] = s.copy()
        return s

    extracted_schema = WalkAndApply(_extract_definitions, apply_before_recurse=False).walk(schema)
    ...
    def _replace_refs(s: core_schema.CoreSchema) -> core_schema.CoreSchema:
        if 'ref' in s:
            return {'type': 'definition-ref', 'schema_ref': s['ref']}
        return s

    schema = WalkAndApply(_replace_refs, apply_before_recurse=False).walk(extracted_schema)
    return {'definitions': list(definitions.values()), 'schema': schema, 'type': 'definitions'}
